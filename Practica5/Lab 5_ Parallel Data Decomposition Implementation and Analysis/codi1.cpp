...
void mandel_simple(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      ...
      int howmany = COLS/num_threads;
      int my_px_start = my_thread*howmany;
      int my_px_end = my_px_start + howmany;
      int reminder = COLS%num_threads;
      for (int py = 0; py < ROWS; py++) {
	    for (int px = my_px_start; px < my_px_end; px++) {
	...
	if((reminder-my_thread)>0) {
		int px = (COLS-1) - my_thread;
		M[py][px] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI, px, py, scale_real, scale_imag, maxiter);
		  if (output2histogram)
		  	#pragma omp atomic
			histogram[M[py][px] - 1]++;
