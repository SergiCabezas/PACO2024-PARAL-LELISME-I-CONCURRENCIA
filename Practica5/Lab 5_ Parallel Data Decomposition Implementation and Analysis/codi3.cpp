...
void mandel_simple(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
   // Calcular
        ...
        for (int i = myid; i < ROWS; i += howmany) {
            for (int j = 0; j < COLS; j++) {
                M[i][j] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI, j, i, scale_real, scale_imag, maxiter);
                if (output2histogram)
                    #pragma omp atomic
                    histogram[M[i][j] - 1]++;
                ...
                if (output2display) {
                    #pragma omp critical
                    {
                        long color = (long)((M[i][j] - 1) * scale_color) + min_color;
